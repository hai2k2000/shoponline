...
export default async function TrackingPage({ searchParams }: { searchParams: Promise<{ code?: string }> }) {
  ...
  const order = code ? await prisma.order.findUnique({ where: { orderCode: code }, include: { customer: true, items: true } }) : null;
  return (
