...
export default function CartPage() {
  ...
  return (
    <StoreShell compact>
      ...
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div>
          <span className="text-sm text-slate-500">{count} sản phẩm</span>
          <strong className="mt-1 block text-2xl">Tạm tính {money(total)}</strong>
          <p className="mt-1 text-xs text-slate-500">Phí giao hàng và mã giảm giá được tính ở bước thanh toán.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {items.length ? <button className="inline-flex min-h-10 items-center justify-center rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50" onClick={() => save([])}>Xóa giỏ</button> : null}
          <StoreButton href="/checkout" disabled={!items.length}>Thanh toán</StoreButton>
        </div>
      </section>
    </StoreShell>
  );
