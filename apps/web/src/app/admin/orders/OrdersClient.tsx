    ...
    <main className="min-h-screen bg-slate-50 p-6 text-slate-950">
      <div className="mx-auto grid max-w-7xl gap-6">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div><p className="text-sm font-semibold text-emerald-700">Admin / Đơn hàng</p><h1 className="text-3xl font-semibold">Quản lý đơn hàng</h1><p className="mt-1 text-sm text-slate-600">Tạo đơn, giữ tồn kho, theo dõi trạng thái và hoàn tất xuất kho.</p></div>
        ...
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] text-left text-sm">
              ...
              <tbody>{filtered.map((row) => <tr key={row.id} className="border-t border-slate-100"><td className="px-4 py-3"><strong>{row.orderCode}</strong><p className="mt-1 text-xs text-slate-500">{dateText(row.createdAt)}</p></td><td className="px-4 py-3">{row.customerName}<p className="mt-1 text-xs text-slate-500">{row.note || ""}</p></td><td className="px-4 py-3">{row.items.map((item) => `${item.productName} x${item.quantity}`).join(", ")}</td><td className="px-4 py-3 font-semibold">{money(row.total)}</td><td className="px-4 py-3">{viPayment(row.paymentStatus)}</td><td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold">{viOrderStatus(row.orderStatus)}</span></td><td className="px-4 py-3"><div className="flex flex-wrap justify-end gap-2"><button className="rounded-lg border border-slate-300 px-3 py-2 font-semibold" onClick={() => setModal({ mode: "detail", row })}>Xem</button><StatusButtons row={row} sessionToken={sessionToken} /></div></td></tr>)}{!filtered.length ? <tr><td className="px-4 py-10 text-center text-slate-500" colSpan={7}>Không có đơn hàng phù hợp.</td></tr> : null}</tbody>
            </table>
          </div>
        </section>
      </div>
      {modal?.mode === "detail" ? <OrderDetail row={modal.row} onClose={() => setModal(null)} /> : modal ? <OrderModal customers={customers} products={products} sessionToken={sessionToken} onClose={() => setModal(null)} /> : null}
    </main>
  );
}
...
function OrderModal({ customers, products, sessionToken, onClose }: { customers: CustomerOption[]; products: ProductOption[]; sessionToken: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4">
      <form action="/api/admin/orders" method="post" className="grid w-full max-w-3xl gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
        <input type="hidden" name="sessionToken" value={sessionToken} />
        <div className="flex items-center justify-between border-b border-slate-100 pb-3"><h2 className="text-xl font-semibold">Tạo đơn hàng</h2><button type="button" className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold" onClick={onClose}>Đóng</button></div>
...
function StatusButtons({ row, sessionToken }: { row: OrderRow; sessionToken: string }) {
  ...
  const options = row.orderStatus === "COMPLETED" ? [["RETURNED", "Trả hàng"]] : [
    ...(row.orderStatus === "NEW" ? [["CONFIRMED", "Xác nhận"]] : []),
    ...(["NEW", "CONFIRMED", "PACKING", "SHIPPING"].includes(row.orderStatus) ? [["COMPLETED", "Hoàn tất"]] : []),
    ...(["NEW", "CONFIRMED", "PACKING"].includes(row.orderStatus) ? [["CANCELLED", "Huỷ"]] : []),
  ];
  return <>{options.map(([status, label]) => <form key={status} action="/api/admin/orders/status" method="post" onSubmit={(event) => { if (["CANCELLED", "RETURNED"].includes(status) && !confirm("Xác nhận đổi trạng thái đơn hàng?")) event.preventDefault(); }}><input type="hidden" name="sessionToken" value={sessionToken} /><input type="hidden" name="id" value={row.id} /><input type="hidden" name="status" value={status} /><button className="rounded-lg border border-slate-300 px-3 py-2 font-semibold" type="submit">{label}</button></form>)}</>;
}
