...
type ModalState = { mode: "create" } | { mode: "detail"; row: ReturnRow } | null;
...
export function ReturnsClient({ rows, orders, sessionToken }: { rows: ReturnRow[]; orders: OrderOption[]; sessionToken: string }) {
  ...
  return (
    <AdminPage>
      <PageHeader eyebrow="Admin / Đơn hàng / Trả hàng" title="Trả hàng và hoàn tiền" description="Quản lý yêu cầu trả hàng, duyệt yêu cầu, nhận hàng hoàn kho và ghi nhận hoàn tiền." action={<><a className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold transition hover:bg-slate-50" href={`/api/admin/returns/export?${exportParams(query, status)}`}>Tai CSV</a><Button onClick={() => setModal({ mode: "create" })}><Plus className="mr-2 size-4" />Tạo yêu cầu</Button></>} />
      ...
      {modal?.mode === "create" ? <ReturnModal orders={orders} sessionToken={sessionToken} onClose={() => setModal(null)} /> : null}
      {modal?.mode === "detail" ? <ReturnDetail row={modal.row} onClose={() => setModal(null)} /> : null}
    </AdminPage>
  );
}
...
function ReturnDetail({ row, onClose }: { row: ReturnRow; onClose: () => void }) { return <ModalShell title={`Yêu cầu ${row.code}`} onClose={onClose} width="max-w-2xl"><div className="grid gap-3 md:grid-cols-2"><Info label="Đơn hàng" value={row.orderCode} /><Info label="Khách hàng" value={row.customer} /><Info label="Trạng thái" value={viStatus(row.status)} /><Info label="Lý do" value={row.reason} /><Info label="Số tiền hoàn" value={money(row.refundAmount)} /><Info label="Ngày tạo" value={dateText(row.createdAt)} /><Info label="Ngày nhận hàng" value={row.receivedAt ? dateText(row.receivedAt) : "-"} /><Info label="Ngày hoàn tiền" value={row.refundedAt ? dateText(row.refundedAt) : "-"} /><Info label="Ghi chú" value={row.note || "-"} wide /></div></ModalShell>; }
