...
type ModalState = { mode: "create" } | { mode: "detail"; row: PromotionRow } | null;
...
export function PromotionsClient({ rows, sessionToken }: { rows: PromotionRow[]; sessionToken: string }) {
  ...
  return (
    <AdminPage>
      <PageHeader eyebrow="Admin / Bán hàng / Khuyến mãi" title="Khuyến mãi" description="Tạo và quản lý mã giảm giá áp dụng ở checkout, theo dõi lượt dùng, giới hạn và thời hạn hiệu lực." action={<><a className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold transition hover:bg-slate-50" href={`/api/admin/promotions/export?${exportParams(query, status)}`}>Tai CSV</a><Button onClick={() => setModal({ mode: "create" })}><Plus className="mr-2 size-4" />Tạo mã</Button></>} />
      ...
      {modal?.mode === "create" ? <PromotionModal sessionToken={sessionToken} onClose={() => setModal(null)} /> : null}
      {modal?.mode === "detail" ? <PromotionDetail row={modal.row} onClose={() => setModal(null)} /> : null}
    </AdminPage>
  );
}
...
function PromotionDetail({ row, onClose }: { row: PromotionRow; onClose: () => void }) { const usage = row.usageLimit ? `${row.usedCount} / ${row.usageLimit}` : `${row.usedCount} / Không giới hạn`; return <ModalShell title={`Mã ${row.code}`} onClose={onClose} width="max-w-2xl"><div className="grid gap-3 md:grid-cols-2"><Info label="Tên chương trình" value={row.name} wide /><Info label="Trạng thái" value={viStatus(row.status)} /><Info label="Loại giảm" value={row.discountType === "PERCENT" ? "Phần trăm" : "Số tiền"} /><Info label="Giá trị giảm" value={row.discountType === "PERCENT" ? `${row.discountValue}%` : money(row.discountValue)} /><Info label="Giảm tối đa" value={row.maxDiscount ? money(row.maxDiscount) : "-"} /><Info label="Đơn tối thiểu" value={money(row.minOrder)} /><Info label="Lượt dùng" value={usage} /><Info label="Ngày bắt đầu" value={row.startsAt ? dateText(row.startsAt) : "-"} /><Info label="Ngày hết hạn" value={row.endsAt ? dateText(row.endsAt) : "-"} /></div></ModalShell>; }
