...
type ModalState = { mode: "create" } | { mode: "edit"; row: UserRow } | { mode: "password"; row: UserRow } | { mode: "detail"; row: UserRow } | null;
...
export function UsersClient({ rows, currentUserId, sessionToken }: { rows: UserRow[]; currentUserId: string; sessionToken: string }) {
  ...
  return (
    <AdminPage>
      <PageHeader eyebrow="Admin / Người dùng" title="Quản lý người dùng" description="Tạo tài khoản, phân vai trò, khóa tài khoản và reset mật khẩu theo đúng phân quyền vận hành." action={<div className="flex flex-wrap justify-end gap-2"><a className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold transition hover:bg-slate-50" href={`/api/admin/users/export?${exportParams(query, role, status)}`}><Download className="mr-2 size-4" />CSV</a><Button onClick={() => setModal({ mode: "create" })}><Plus className="mr-2 size-4" />Tạo người dùng</Button></div>} />
      ...
      <DataPanel>
        <table className="w-full min-w-[1040px] text-left text-sm">
          ...
          <tbody>{visibleRows.map((row) => <tr key={row.id} className="border-t border-slate-100 align-top hover:bg-slate-50/70"><td className="px-4 py-3"><strong>{row.name}</strong>{row.id === currentUserId ? <p className="mt-1 text-xs font-semibold text-emerald-700">Tài khoản hiện tại</p> : null}</td><td className="px-4 py-3 text-slate-600">{row.email}</td><td className="px-4 py-3">{viRole(row.role)}</td><td className="px-4 py-3"><StatusBadge tone={statusTone(row.status)}>{viStatus(row.status)}</StatusBadge></td><td className="px-4 py-3">{dateText(row.createdAt)}</td><td className="px-4 py-3"><div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => setModal({ mode: "detail", row })}>Xem</Button><Button variant="outline" onClick={() => setModal({ mode: "edit", row })}>Sửa</Button><Button variant="outline" onClick={() => setModal({ mode: "password", row })}>Reset mật khẩu</Button>{row.status !== "ARCHIVED" && row.id !== currentUserId ? <ArchiveButton row={row} sessionToken={sessionToken} /> : null}</div></td></tr>)}</tbody>
        </table>
      ...
      {modal?.mode === "detail" ? <UserDetail row={modal.row} currentUserId={currentUserId} onClose={() => setModal(null)} /> : null}
...
function UserDetail({ row, currentUserId, onClose }: { row: UserRow; currentUserId: string; onClose: () => void }) { return <ModalShell title={row.name} onClose={onClose} width="max-w-2xl"><div className="grid gap-3 md:grid-cols-2"><Info label="Email" value={row.email} /><Info label="Vai trò" value={viRole(row.role)} /><Info label="Trạng thái" value={viStatus(row.status)} /><Info label="Tài khoản hiện tại" value={row.id === currentUserId ? "Có" : "Không"} /><Info label="Ngày tạo" value={dateTimeText(row.createdAt)} /><Info label="Cập nhật cuối" value={dateTimeText(row.updatedAt)} /><Info label="User ID" value={row.id} wide /></div></ModalShell>; }
...
function dateTimeText(value: string) { return new Intl.DateTimeFormat("vi-VN", { dateStyle: "short", timeStyle: "short" }).format(new Date(value)); }
...
function exportParams(query: string, role: string, status: string) { const params = new URLSearchParams(); if (query.trim()) params.set("search", query.trim()); if (role) params.set("role", role); if (status) params.set("status", status); return params.toString(); }
