...
export function ExpensesClient({ rows }: { rows: ExpenseRow[] }) {
...
function ArchiveButton({ row }: { row: ExpenseRow }) { const [pending, startTransition] = useTransition(); return <button className="rounded-lg border border-red-200 px-3 py-2 font-semibold text-red-700 disabled:opacity-60" disabled={pending} onClick={() => { if (!confirm("Lưu trữ chi phí này?")) return; const formData = new FormData(); formData.set("id", row.id); startTransition(async () => { await archiveExpenseAction(formData); window.location.reload(); }); }}>{pending ? "Đang lưu..." : "Lưu trữ"}</button>; }
