...
import { AdminPage, Button, DataPanel, EmptyState, FilterBar, ModalShell, PageHeader, Pagination, SearchField, SelectField, StatCard, StatusBadge, type Tone } from "@/components/admin/ui";
...
export function NotificationsClient({ rows, sessionToken }: { rows: NotificationRow[]; sessionToken: string }) {
  ...
  return (
    <AdminPage>
      <PageHeader eyebrow="Admin / Hệ thống / Thông báo" title="Thông báo" description="Theo dõi cảnh báo và sự kiện nghiệp vụ cần xử lý, ưu tiên các mục chưa đọc hoặc mức cảnh báo cao." action={<><a className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold transition hover:bg-slate-50" href={`/api/admin/notifications/export?${exportParams(query, mode, level)}`}>Tai CSV</a><form action="/api/admin/notifications/read-all" method="post"><input type="hidden" name="sessionToken" value={sessionToken} /><Button type="submit">Đánh dấu đã đọc</Button></form></>} />
      ...
      {selected ? <NotificationDetail row={selected} onClose={() => setSelected(null)} /> : null}
    </AdminPage>
  );
}

function NotificationDetail({ row, onClose }: { row: NotificationRow; onClose: () => void }) { return <ModalShell title={row.title} onClose={onClose} width="max-w-2xl"><div className="grid gap-3 md:grid-cols-2"><Info label="Mức" value={viLevel(row.level)} /><Info label="Trạng thái" value={row.readAt ? "Đã đọc" : "Chưa đọc"} /><Info label="Ngày tạo" value={dateText(row.createdAt)} /><Info label="Ngày đọc" value={row.readAt ? dateText(row.readAt) : "-"} /><Info label="Nghiệp vụ" value={row.entityType || "Hệ thống"} /><Info label="Entity ID" value={row.entityId || "-"} /><Info label="Nội dung" value={row.message || "-"} wide /></div></ModalShell>; }
...
function exportParams(query: string, mode: string, level: string) { const params = new URLSearchParams(); if (query.trim()) params.set("search", query.trim()); if (mode) params.set("mode", mode); if (level) params.set("level", level); return params.toString(); }
